...
class CaliFlexiv():
    ...
    def Calculate_core(self, left2camera, right2camera, right2world,right2turntable,depth2rgb, type="depth"):
        """Calculate transform matrix.
        Args:
            left2camera: left robot to Global rgb camera transform.
            right2camera: right robot to Global rgb camera transform.
            right2world:  right robot to world transform.
            depth2rgb: depth camera to rgb camera transform.
            type: two choices: rgb or depth.
        Return:
        ...
        depth2rgb = self.transform_to_4x4(depth2rgb)
        rgb2depth = np.linalg.inv(depth2rgb)

        # right robot to world
        right2rgb = self.transform_to_4x4(right2camera)

        if type == "rgb":
            right2camera = right2rgb
        elif type == "depth":
            right2camera = right2rgb.dot(rgb2depth)

        right2world = self.transform_to_4x4(right2world)
        world2right = np.linalg.inv(right2world)

        # left robot to world
        left2rgb = self.transform_to_4x4(left2camera)

        if type == "rgb":
            left2camera = left2rgb
        elif type == "depth":
            left2camera = left2rgb.dot(rgb2depth)
        ...
        # global camera to world (rgb or depth)
...
if __name__ == "__main__":
    ...
    depth_to_rgb_calibration_path = osp.join(__VERSION_CANDIDATES_DIR_, __VERSION__,'depth_camera_to_rgb_camera_cali_4x4.json')
    # read from json file
    ...
    depth2rgb = cal.read_calibration_json(depth_to_rgb_calibration_path)

    # Defined for Robotics
    type = "depth"  # or "rgb"
    camera2world, left2world, right2world, turntable2world= cal.Calculate_core(left2camera, right2camera, right2world, right2turntable,depth2rgb,type = type)
    ...
    cal.save_calibration_json(osp.join(output_dir, 'world_to_camera_transform.json'), camera2world)
    cal.save_calibration_json(osp.join(output_dir, 'world_to_left_robot_transform.json'), left2world)
    cal.save_calibration_json(osp.join(output_dir, 'world_to_right_robot_transform.json'), right2world)
    cal.save_calibration_json(osp.join(output_dir, 'world_to_turntable_transform.json'), turntable2world)
