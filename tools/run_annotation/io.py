...
class AnnotatorNetworkIO(AnnotatorIO):
    ...
    def scan_log_dir(self) -> Tuple[List[str], List[str], List[str], Optional[Exception]]:
        unp_extra_filter = {
            '$and': [
                ...
                {
                    f"metadata.{self.opt.raw_log_namespace}": {
                        "$exists": True
                    }
                }
            ]
            + [
                {
                    f"metadata.{self.opt.raw_log_namespace}.tag":
                        self.opt.root_dir
                }
            ] if self.opt.root_dir != "default" else []
        }
        p_extra_filter = {
            '$and': [
                self.opt.extra_filter if self.opt.extra_filter is not None else {},
    ...
    def write_annotation(
        ...
        json=log_point.to_dict()
        def process_infinity_and_nan(dictionary, inf_num):
            if isinstance(dictionary, dict):
                for key, value in dictionary.items():
                    process_infinity_and_nan(value, inf_num=inf_num)
                    if isinstance(value, dict):
                        if len(value) == 1:
                            for key0, value0 in value.items():
                                if key0 == "$numberDouble":
                                    if value0 == "Infinity":
                                        dictionary[key] = float(inf_num)
                                    elif value0 == "-Infinity":
                                        dictionary[key] = float(-inf_num)
                                    elif value0 == "NaN":
                                        dictionary[key] = float(inf_num) # TODO: better way to handle NaN
            elif isinstance(dictionary, list):
                for idx, i in enumerate(dictionary):
                    process_infinity_and_nan(i, inf_num=inf_num)
                    if isinstance(i, dict):
                        if len(i) == 1:
                            for key0, value0 in i.items():
                                if key0 == "$numberDouble":
                                    if value0 == "Infinity":
                                        dictionary[idx] = float(inf_num)
                                    elif value0 == "-Infinity":
                                        dictionary[idx] = float(-inf_num)
                                    elif value0 == "NaN":
                                        dictionary[idx] = float(inf_num) # TODO: better way to handle NaN
        ...
        process_infinity_and_nan(json, inf_num=1e10)
