...
import os
import sys
from common.datamodels import ObjectTypeDef
...
def main(args):
    opt = AnnotationConfig(
        ...
        object_type=ObjectTypeDef.from_string(args.object_type),
        K=args.K,
        ...
        multi_pose_num=args.multi_pose_num,
...
def as_complete(args):
    err = None
    if args.raw_log_namespace is None:
        args.raw_log_namespace = py_cli_interaction.must_parse_cli_string("input raw log namespace")
    ...
    if args.annotation_type is None:
        args.annotation_type = py_cli_interaction.must_parse_cli_string("input annotation type", default_value="")
...
def entrypoint(argv):
    ...
    parser.add_argument('--object_type', type=str, default='tshirt_short', help='object type')
    ...
    parser.add_argument('--raw_log_namespace', type=str, default=None, help="namespace of original log")
    ...
    parser.add_argument('--annotation_type', type=str, default=None, 
                        choices=["real_finetune", "new_pipeline_supervised", "new_pipeline_supervised_classification_detection", "new_pipeline_finetune", "new_pipeline_finetune_sort"], help='type of annotation')
    parser.add_argument('--multi_pose_num', type=int, default=-1, help='number of poses to annotate, -1 or 0 for unlimited')
    parser.add_argument('--keypoint_num', type=int, default=-1, help='number of keypoints to annotate, -1 or 0 for unlimited')
