...
class vCamera:
    ...
    def open(self) -> List[int]:
        ...
        errors.append(mvsdk.CameraSetTriggerMode(self.h_camera, self.trigger))  # the camera mode is set to continuous capture
        if True:  # conti or soft
            errors.append(mvsdk.CameraSetAeState(self.h_camera,
                                                 self.auto_exposure))  # manual exposure -> 0 AUTO EXPOSURE -> 1，exposure time
            if not self.auto_exposure:
                errors.append(
                    mvsdk.CameraSetFrameSpeed(self.h_camera, self.frame_speed))  # set frame speed to 0 (slow) when long exposure
                errors.append(mvsdk.CameraSetExposureTime(self.h_camera, int(self.exposure_ms * 1000)))
            mvsdk.CameraSetWbMode(self.h_camera, 0)

        errors.append(mvsdk.CameraPlay(self.h_camera))  # start the internal thread of the SDK for image acquisition
    ...
    def read(self) -> Union[np.ndarray, None]:
        ...
        # the image is stored in self.p_frame_buffer
...
if __name__ == '__main__':
    __TEST_MODE__ = not py_cli_interaction.must_parse_cli_bool("Calibration Mode (Gray image)?", default_value=True)
    ...
    # SAVE_PATH_BASE = '/home/xuehan/Desktop/Output_short_RGB'
    ...
    with camera as c:
        with tqdm.tqdm(range(1)) as pbar:
            ...
            while (cmd & 0xFF) != ord('q'):
                ...
                if (cmd & 0xFF) == ord('\r'):
                    if not __TEST_MODE__:                   
                        fname = os.path.join(SAVE_PATH_BASE, 'image{}.png'.format(capture_cnt))
                    else:
